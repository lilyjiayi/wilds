    ...
    def __init__(
        ...
        if use_four:
            ...
            for domain in SENTRY_DOMAINS:
                curr_metadata = metadata_df.loc[metadata_df["domain"] == domain]
                if metadata_result is None:
                    metadata_result = curr_metadata
                else:  metadata_result = pd.concat([metadata_result, curr_metadata])
            metadata_df = metadata_result
        ...
        possible_metadata_values = {
            "domain": DOMAIN_NET_DOMAINS if ((not use_four) and (not use_sentry)) else SENTRY_DOMAINS,
